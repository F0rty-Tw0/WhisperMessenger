...
return function()
  ...
  assert(
    window.contacts.scrollBar.point[2] == window.contacts.scrollFrame,
    "expected contacts scrollbar to align with the contacts viewport"
  )
  ...
  assert(
    window.contacts.scrollFrame.scrollChild == window.contacts.content,
    "expected contacts content to be wired as scroll child"
  )
  ...
  assert(
    window.conversation.transcript.scrollBar.point[3] == "TOPRIGHT",
    "expected transcript scrollbar relative point"
  )
  assert(
    window.conversation.transcript.scrollBar.point[4] == 0,
    "expected transcript scrollbar indicators to stay inside the window"
  )
  ...
  assert(
    window.conversation.transcript.scrollFrame:GetVerticalScroll() == 0,
    "expected transcript scroll to clamp at the first message"
  )
  ...
  assert(
    transitionWindow.conversation.transcript.scrollBar.shown == false,
    "expected transcript scrollbar to stay hidden before overflow"
  )
  ...
  assert(
    transitionWindow.conversation.transcript.scrollBar.shown == true,
    "expected transcript scrollbar to appear after transcript overflow"
  )
  assert(
  ...
  assert(
    transitionWindow.conversation.transcript.scrollBar.shown == false,
    "expected transcript scrollbar to hide after transcript overflow clears"
  )
  assert(
    transitionWindow.conversation.transcript.scrollFrame.width == fullTranscriptWidth,
  ...
  assert(
    exactFitView.scrollBar.shown == false,
    "expected scrollbar to stay hidden when content exactly fits the viewport"
  )
  ...
  assert(
    resizedView.content.width == resizedView.scrollFrame.width,
    "expected resized content width to match the viewport width"
  )
  ...
  assert(
    lateInitView.scrollBar.height == 80,
    "expected zero-height recovery to size the scrollbar to the live viewport height"
  )
  ...
  assert(
    lateInitView.scrollBar.shown == true,
    "expected zero-height recovery to show the scrollbar for overflowing content"
  )
