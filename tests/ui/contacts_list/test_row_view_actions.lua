...
return function()
  ...
  do
    ...
    local onLeave = row.scripts.OnLeave
    onEnter(row)
    -- Simulate mouse moving to child button: row.IsMouseOver still true
    row.mouseOver = true
    onLeave(row)
    assert(row.pinButton:IsShown() == true, "pinButton should stay visible when mouse is over child")
    assert(row.removeButton:IsShown() == true, "removeButton should stay visible when mouse is over child")
    -- Now mouse truly leaves
    row.mouseOver = false
    onLeave(row)
