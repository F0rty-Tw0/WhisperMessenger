...
return function()
  ...
  -- test_badge_hidden_without_peer_addon
  do
    ...
    assert(view.headerAddonBadge:IsShown() == false, "badge should be hidden when peerHasAddon is falsy")
  end

  -- test_badge_shown_with_peer_addon
  do
    local contact = { displayName = "Arthas", classTag = "HUNTER", factionName = "Horde", peerHasAddon = true }
    local view = HeaderView.Create(factory, pane, contact)
    HeaderView.Refresh(view, contact, nil, nil)
