...
return function()
  ...
  -- Test 7: InChatMessagingLockdown blocks send
  -- FlavorCompat.InChatMessagingLockdown() reads _G.C_ChatInfo at call time;
  -- we stub it here and force a FlavorCompat reload so the module uses the stub.
  rawset(runtime, "isChatMessagingLocked", function()
    return false
  end)
  rawset(_G, "InCombatLockdown", function()
  ...
  package.loaded["WhisperMessenger.Core.Bootstrap.SendHandler"] = nil
  local SH2 = require("WhisperMessenger.Core.Bootstrap.SendHandler")
  runtime.sendStatusByConversation = {}
  runtime.pendingOutgoing = {}
  refreshCalls = 0
  sentMessages = {}

  local chatLockResult = SH2.HandleSend(runtime, payload, refreshWindow)
  assert(chatLockResult == false, "expected send to be blocked during chat messaging lockdown")
  assert(#sentMessages == 0, "should not send during chat messaging lockdown")
  assert(refreshCalls == 1, "should refresh to show chat lockdown status")

  local chatLockStatus = runtime.sendStatusByConversation[payload.conversationKey]
  assert(chatLockStatus ~= nil, "expected status to be set during chat messaging lockdown")
  assert(
    chatLockStatus.status == "Lockdown",
    "expected 'Lockdown' status during chat messaging lockdown, got: " .. tostring(chatLockStatus.status)
  )
  local chatLockConv = runtime.store.conversations[payload.conversationKey]
  assert(chatLockConv ~= nil, "expected conversation to exist after chat lockdown block")
  local lastChatLockMsg = chatLockConv.messages[#chatLockConv.messages]
  assert(lastChatLockMsg.delivery == "blocked", "expected blocked delivery marker for chat lockdown")
  assert(lastChatLockMsg.blockedReason == "Lockdown", "expected 'Lockdown' blocked reason for chat lockdown")

  -- Test 8: InChatMessagingLockdown false -> send proceeds
  rawset(_G, "C_ChatInfo", {
    InChatMessagingLockdown = function()
      return false
    end,
  ...
  package.loaded["WhisperMessenger.Core.FlavorCompat"] = nil
  package.loaded["WhisperMessenger.Core.Bootstrap.SendHandler"] = nil
  local SH3 = require("WhisperMessenger.Core.Bootstrap.SendHandler")
  runtime.sendStatusByConversation = {}
  runtime.pendingOutgoing = {}
  refreshCalls = 0
  sentMessages = {}

  local chatUnlockedResult = SH3.HandleSend(runtime, payload, refreshWindow)
  assert(chatUnlockedResult == true, "expected send to succeed when InChatMessagingLockdown returns false")
  assert(#sentMessages == 1, "should send when chat messaging lockdown is inactive")
