...
local HeaderView   = ns.ConversationPaneHeaderView
  or require("WhisperMessenger.UI.ConversationPane.HeaderView")
...
function ConversationPane.Create(factory, parent, selectedContact, conversation)
  ...
  if transcript.text.SetMultiLine     then transcript.text:SetMultiLine(true)   end
  if transcript.text.SetAutoFocus     then transcript.text:SetAutoFocus(false)  end
  if transcript.text.EnableMouse      then transcript.text:EnableMouse(true)    end
  if transcript.text.SetHyperlinksEnabled then transcript.text:SetHyperlinksEnabled(true) end
  ...
  if transcript.scrollFrame and transcript.scrollFrame.SetScript then
    ...
    transcript.scrollFrame:SetScript("OnMouseWheel", function(self, delta)
      if originalOnWheel then originalOnWheel(self, delta) end
  ...
  if transcript.scrollBar and transcript.scrollBar.SetScript then
    ...
    transcript.scrollBar:SetScript("OnValueChanged", function(self, value)
      if originalOnValue then originalOnValue(self, value) end
      checkLoadMoreMessages()
    end)
  end
