...
- **Reply-key override now respects the user's REPLY keybinding.** `ReplyKeyBinder` previously hardcoded `"R"` as the override key, which stole keystrokes from users who had rebound `R` to an ability and moved REPLY to a different key (e.g. `T`). The binder now queries `GetBindingKey("REPLY")` and overrides every key slot bound to the REPLY action (WoW allows up to two). If REPLY is unbound entirely, the binder skips the override and does not take over an arbitrary key. A new `UPDATE_BINDINGS` lifecycle hook re-runs `syncReplyKey` so remapping REPLY in the in-game keybindings UI takes effect immediately without `/reload`. New regression tests cover: user-bound non-R key, multi-slot bindings, unbound REPLY, empty-string slots, live rebind, and live unbind.
...
- **Classic / TBC Anniversary clients no longer throw `Attempt to register unknown event "ADDON_RESTRICTION_STATE_CHANGED"` at load.** `ADDON_RESTRICTION_STATE_CHANGED` is a 12.0 Midnight (Retail-only) event, but `AddonEventFrame.Install` was calling `loadFrame:RegisterEvent(eventName)` directly in the lifecycle-event loop, which throws on pre-12.0 clients. The registration now goes through a `pcall`-guarded helper that mirrors the existing `registerEventIfSupported` pattern in `EventBridge.lua` — unknown events are skipped silently and the rest of the lifecycle list continues registering. `Core/Constants.lua`'s `LIFECYCLE_EVENT_NAMES` still lists the event unchanged so Retail keeps the 12.0 integration. New `tests/core/test_addon_event_frame_classic.lua` regression test simulates the Classic throw and asserts both neighbouring events still register.
- **Message transcript no longer shows a scrollbar for non-overflowing conversations.** Two linked issues: (1) `ScrollView.Metrics.RefreshMetrics` computed `hasOverflow = contentHeight > viewportHeight`, and `Metrics.GetRange` returned `contentHeight - viewportHeight`, without guarding `viewportHeight > 0`. During certain early lifecycle moments (frame not yet laid out / hidden) `scrollFrame:GetHeight()` returns 0, so any non-empty content read as overflow and the scrollbar would show. Both paths now short-circuit to "no overflow" when `viewportHeight <= 0`. (2) `GetRange` trusted WoW's `GetVerticalScrollRange()` first, which can report a stale range after the scroll child is resized between conversations; the content-vs-viewport math is now authoritative and `GetVerticalScrollRange` is only consulted as a fallback.
- **Contact list no longer auto-pages to the end on first open.** `ContactsController.checkLoadMore` triggered `loadMore()` whenever `offset >= range - ROW_HEIGHT`. With the default 11-row rendering window (10 base + 1 safety margin) the initial overflow is ~34px — less than one row — so the trigger threshold went negative and every `Navigation.Sync`-initiated `SetValue(0)` callback satisfied it, pumping `visibleCount` up to `#contacts` before the user ever scrolled. The check now also requires `offset > 0` so spurious sync-driven callbacks don't cascade into pagination.
- **Contact rows no longer bleed past the contacts pane border.** Two converging issues: (1) `ScrollView.Factory` creates the `ScrollFrame` with `SetScrollChild(content)`, but in WoW that does not clip descendant frames — only the scroll child's own textures — so the contact-row `Button` frames inside the scroll child painted over the pane border. (2) The contacts `scrollFrame` was sized from `contactsListHeight = height - TOP_BAR_HEIGHT - searchTotalHeight`, but `contactsPane` is dual-anchored `TOPLEFT (6, -24)` / `BOTTOMLEFT (7, 6)` so its real height is `frame_height - 30`, leaving the scrollFrame about 8px taller than the pane. Fixed by calling `scrollFrame:SetClipsChildren(true)` in `ScrollView.Factory` and adding a `BOTTOMRIGHT` dual-anchor to the pane in `LayoutBuilder.Build` and `LayoutBuilder/Apply.Relayout` (SetSize is kept for fake_ui parity; the dual-anchor is a WoW-only safety net that pins the viewport to the pane).
- **Selected contact stays highlighted the moment the window opens.** `ContactsController.refresh` was called internally by `fillViewport` and `loadMore` as `refresh(nil, nil, ...)` where the second `nil` meant "keep current selection" — but the implementation forwarded that `nil` straight into `ContactsList.SetSelected`, clearing the highlight. On window open, `fillViewport` ran before the first bootstrap-level `refreshWindow`, so the selected row rendered plain and only lit up after a later event-driven refresh. The controller now caches the last applied `selectedConversationKey` and reuses it whenever a caller passes `nil`, matching the comment's original intent.
- **"WM" window title visible again under Shadowlands and Azeroth.** `ChromeBuilder`'s modern-chrome title FontString was created on the outer `frame` (OVERLAY), but `titleBarBg` is a texture on the `titleBar` child frame (ARTWORK). Child-frame layers render on top of parent-frame layers at the same frame level, so the title was painted *under* `titleBarBg`. Midnight (`surface_chrome` alpha 0.60) and Draenor (0.76) let the title bleed through; Shadowlands (0.90) mostly hid it, and Azeroth (1.00) hid it completely. Moved the title FontString onto `titleBar` (and reanchored `TOPLEFT` accordingly) so it sits on top of `titleBarBg` regardless of preset alpha.
