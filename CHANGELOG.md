...
- Fixed: the Native WoW HUD window now has a cleaner layout: no empty strip above the chat, one even background behind both sides, and the full addon name in the title.
- Native WoW HUD now uses the game's own look inside the window too: a standard search box, game-style Whispers / Groups tabs, a classic message box, and red-gold buttons.
- Native WoW HUD: the "Start a new conversation" and copy-message popups now use the game's standard dialog look.
